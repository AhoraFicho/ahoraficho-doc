---
layout: home
title: Inicio
nav_order: 1
description: "Documentación oficial de AhoraFicho.es - Sistema de Control Horario Digital"
permalink: /
---

# Bienvenido a la Documentación de AhoraFicho.es
{: .fs-9 }

AhoraFicho.es es la solución completa para el control horario digital que cumple con el Real Decreto-ley 8/2019 y facilita la gestión del tiempo de trabajo de tu empresa.
{: .fs-6 .fw-300 }

[Empezar ahora](/primeros-pasos/){: .btn .btn-primary .fs-5 .mb-4 .mb-md-0 .mr-2 }
[Probar Demo](https://democompleta.ahoraficho.es/Identity/Account/AutoLogin?username=comercial@solutions2az.net&password=Demo@2025){: .btn .fs-5 .mb-4 .mb-md-0 target="_blank" }

---

## ¿Qué es AhoraFicho?

AhoraFicho.es es una plataforma web de **control horario digital** diseñada específicamente para cumplir con la normativa española de registro de jornada laboral. Permite a las empresas gestionar de forma eficiente los fichajes de sus empleados, ausencias, vacaciones y mucho más.

### Características principales

- ✅ **Control horario digital** conforme al RD 8/2019
- 📱 **Multiplataforma**: Web, iOS y Android
- 📍 **Geolocalización** opcional para fichajes
- 🔒 **Múltiples métodos de fichaje**: Web, App, PIN, QR, RFID
- 📊 **Reportes completos** para inspecciones de trabajo
- 🏢 **Sistema modular** adaptable a cada empresa
- ⏰ **Gestión de horarios** flexibles y turnos rotativos
- 🌴 **Control de vacaciones** y ausencias

---

## Módulos disponibles

AhoraFicho cuenta con **7 módulos** que puedes activar según las necesidades de tu empresa:

| Módulo | Descripción | Estado |
|:-------|:------------|:-------|
| **Fichajes** | Control de entrada/salida de empleados | Siempre activo |
| **Vacaciones y Ausencias** | Gestión de permisos y días libres | Siempre activo |
| **Gastos** | Control de gastos de empleados | Opcional |
| **Imputaciones** | Seguimiento de proyectos y tareas | Opcional |
| **Turnos** | Gestión de turnos rotativos | Opcional |
| **Servicios** | Control de servicios externos (limpiezas, mantenimiento) | Opcional |
| **Documentos** | Distribución y acuse de recibo de documentos | Opcional |

---

## Guías rápidas por rol

<div class="code-example" markdown="1">

### 👤 Empleado
- [Mi primer fichaje](/guias-por-rol/empleado/como-fichar/)
- [Solicitar vacaciones](/guias-por-rol/empleado/solicitar-vacaciones/)
- [¿Olvidé fichar?](/guias-por-rol/empleado/olvide-fichar/)

### 👨‍💼 Manager
- [Validar ausencias del equipo](/guias-por-rol/manager/validar-ausencias/)
- [Aprobar cambios de fichaje](/guias-por-rol/manager/validar-cambios-fichaje/)
- [Consultar reportes del equipo](/guias-por-rol/manager/reportes-equipo/)

### 🔧 Administrador
- [Dar de alta empleados](/guias-por-rol/administrador/dar-alta-empleados/)
- [Crear y asignar horarios](/guias-por-rol/administrador/crear-horarios/)
- [Configurar turnos rotativos](/guias-por-rol/administrador/configurar-turnos-rotativos/)
- [Desactivar usuarios](/guias-por-rol/administrador/desactivar-usuarios/)
- [Informe para inspección de trabajo](/reportes/informe-inspeccion-trabajo/)

</div>

---

## Primeros pasos

{: .note }
> Si es tu primera vez usando AhoraFicho, te recomendamos empezar por la sección [Primeros Pasos](/primeros-pasos/).

1. **[Registro y onboarding](/primeros-pasos/registro-y-onboarding/)** - Cómo se configura tu empresa
2. **[Acceso a la plataforma](/primeros-pasos/acceso-a-la-plataforma/)** - Login y navegación
3. **[Primer fichaje](/primeros-pasos/primer-fichaje/)** - Cómo realizar tu primer registro
4. **[Descargar app móvil](/primeros-pasos/descarga-app-movil/)** - Instalar en iOS/Android

---

## Soporte y ayuda

¿No encuentras lo que buscas? Consulta nuestra sección de [Preguntas Frecuentes](/preguntas-frecuentes/) o contacta con nuestro equipo de soporte:

- 📧 Email: [soporte@ahoraficho.es](mailto:soporte@ahoraficho.es)
- 🌐 Web: [www.ahoraficho.es](https://www.ahoraficho.es)
- 🏢 Solutions2AZ: [www.solutions2az.net](https://www.solutions2az.net)

---

{: .important }
> **Cumplimiento normativo**: AhoraFicho.es cumple con el Real Decreto-ley 8/2019 de medidas urgentes de protección social y de lucha contra la precariedad laboral en la jornada de trabajo. [Más información sobre normativa legal](/normativa-legal/).